...
def get_loader(args):
    ...
    train_data = MiniImageNetDataset(args.root_dir, args.train_file_path, transform = transform, image_size=args.image_size)
    train_loader = DataLoader(dataset = train_data, batch_size = args.batch_size, shuffle=True, num_workers=8, collate_fn=collate_fn)

    val_data = MiniImageNetDataset(args.root_dir, args.val_file_path, transform = transform, image_size=args.image_size)
    val_loader = DataLoader(dataset = val_data, batch_size = args.batch_size, shuffle=True, collate_fn=collate_fn)

    test_data = MiniImageNetDataset(args.root_dir, args.test_file_path, transform = transform, image_size=args.image_size)
