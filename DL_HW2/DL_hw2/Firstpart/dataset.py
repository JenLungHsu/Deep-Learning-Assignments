...
class MiniImageNetDataset(Dataset):
    def __init__(self, root_dir, file_path, transform = None, image_size = 256, diff_channel = 'RGB'):
        super().__init__()  
    ...
    def __getitem__(self, index):
        ...
        if self.diff_channel == 'RGB':
            pass
        elif self.diff_channel == 'RG':
            img = img[:, :, :2]  # RG
        elif self.diff_channel == 'GB':
            img = img[:, :, 1:]  # GB
        elif self.diff_channel == 'R':
            img = img[:, :, :1]  # R
        elif self.diff_channel == 'G':
            img = img[:, :, 1:2]  # G
        elif self.diff_channel == 'B':
            img = img[:, :, 2:3]  # B
